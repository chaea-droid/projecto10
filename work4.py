...
# ----------------------------
# V2 Grading Prompt (English, force JSON output)
# ----------------------------
judge_prompt = """
You are a grader.
Evaluate the student's answer against the question and reference answer according to a rubric.
For each category (Accuracy, Feedback Specificity, etc.), give a score from 0 to 5
and provide short, constructive feedback.
Output must be strictly in JSON format.

Example output:
{
  "Accuracy": {"score": 4, "feedback": "Core concept is correct but lacks detail"},
  "Feedback Specificity": {"score": 3, "feedback": "Needs more context or examples"},
  "Total": 7
}
"""
...
if __name__ == "__main__":
    ...
    # ----------------------------
    # Save to CSV (English V2 Results)
    # ----------------------------
    with open("grading_results_v2.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
